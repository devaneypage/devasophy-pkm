...
export async function importBooksFromCSV(userId: number, csvData: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  const progressIdx = headers.indexOf("reading progress");
  ...
  for (let i = 1; i < lines.length; i++) {
    try {
      ...
      const bookData = {
        title,
        author: authorIdx !== -1 ? values[authorIdx] : "",
        status: (statusIdx !== -1 ? values[statusIdx] : "want_to_read") as "reading" | "completed" | "want_to_read",
        rating: (ratingIdx !== -1 ? parseFloat(values[ratingIdx]).toString() : "0") as string,
        readingProgress: progressIdx !== -1 ? parseInt(values[progressIdx]) || 0 : 0,
        notes: notesIdx !== -1 ? values[notesIdx] : "",
        tags: tagsIdx !== -1 ? values[tagsIdx] : "",
        coverColor: "#E84D20",
      };

      const result = await createBook(userId, bookData);
      if (result && 'insertId' in result) {
        imported.push({ ...bookData, id: result.insertId });
      }
    } catch (error) {
...
export async function importNotebookEntriesFromCSV(userId: number, csvData: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  const authorIdx = headers.indexOf("author");
  const workIdx = headers.indexOf("work");
  const sourceTypeIdx = headers.indexOf("source type");
  const locationIdx = headers.indexOf("location");
  const noteIdx = headers.indexOf("note");
  const tagsIdx = headers.indexOf("tags");
  const collectionsIdx = headers.indexOf("collections");
  ...
  for (let i = 1; i < lines.length; i++) {
    try {
      ...
      const entryData = {
        text,
        author: authorIdx !== -1 ? values[authorIdx] : "",
        work: workIdx !== -1 ? values[workIdx] : "",
        sourceType: sourceTypeIdx !== -1 ? values[sourceTypeIdx] : "book",
        location: locationIdx !== -1 ? values[locationIdx] : "",
        note: noteIdx !== -1 ? values[noteIdx] : "",
        tags: tagsIdx !== -1 ? values[tagsIdx] : "",
        collections: collectionsIdx !== -1 ? values[collectionsIdx] : "",
        favorite: false,
      };

      const result = await createNotebookEntry(userId, entryData);
      if (result && 'insertId' in result) {
        imported.push({ ...entryData, id: result.insertId });
      }
    } catch (error) {
      errors.push(`Row ${i + 1}: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
...
export async function exportBooksAsJSON(userId: number, filters?: any) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const books = await listBooks(userId, filters);
...
export async function exportNotebookEntriesAsJSON(userId: number, filters?: any) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const entries = await getNotebookEntries(userId, filters);
  
  return {
    format: "json",
    filename: `notebook_export_${new Date().toISOString().split("T")[0]}.json`,
    data: entries,
    metadata: {
      exportDate: new Date().toISOString(),
...
export async function exportBooksAsMarkdown(userId: number, filters?: any) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const books = await listBooks(userId, filters);
  const template = filters?.template ?? "annotated";
  
  let markdown = `# ${template === "catalog" ? "Book Catalog" : "Book Collection"}\n\n`;
  markdown += `**Export Date:** ${new Date().toLocaleDateString()}\n`;
  markdown += `**Total Books:** ${books.length}\n\n`;

  const booksByStatus = new Map<string, any[]>();
  for (const book of books) {
    const status = book.status || "unread";
    if (!booksByStatus.has(status)) {
      booksByStatus.set(status, []);
    }
    booksByStatus.get(status)!.push(book);
  }

  for (const [status, statusBooks] of Array.from(booksByStatus.entries())) {
    markdown += `## ${status.charAt(0).toUpperCase() + status.slice(1)} (${statusBooks.length})\n\n`;
    for (const book of statusBooks) {
      markdown += `### ${book.title}\n`;
      if (book.author) markdown += `**Author:** ${book.author}\n`;
      if (book.rating) markdown += `**Rating:** ${"⭐".repeat(Math.round(parseFloat(book.rating)))}\n`;
      if (book.readingProgress) markdown += `**Progress:** ${book.readingProgress}%\n`;
      if (template === "annotated" && book.notes) markdown += `**Notes:** ${book.notes}\n`;
      if (book.tags) markdown += `**Tags:** ${book.tags.split(",").map((t: string) => `\`${t.trim()}\``).join(" ")}\n`;
      markdown += `\n`;
    }
  }
  ...
  return {
    format: "markdown",
    filename: `books_export_${new Date().toISOString().split("T")[0]}.md`,
    content: markdown,
    metadata: {
      exportDate: new Date().toISOString(),
      count: books.length,
...
export async function exportNotebookEntriesAsMarkdown(userId: number, filters?: any) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const entries = await getNotebookEntries(userId, filters);
  const template = filters?.template ?? "reader";
  
  let markdown = `# ${template === "research" ? "Research Notebook" : "Notebook Entries"}\n\n`;
  markdown += `**Export Date:** ${new Date().toLocaleDateString()}\n`;
  markdown += `**Total Entries:** ${entries.length}\n\n`;

  const entriesByCollection = new Map<string, any[]>();
  for (const entry of entries) {
    const collections = entry.collections ? entry.collections.split(",").map((c: string) => c.trim()) : ["Uncategorized"];
    for (const collection of collections) {
      if (!entriesByCollection.has(collection)) {
        entriesByCollection.set(collection, []);
      }
      entriesByCollection.get(collection)!.push(entry);
    }
  }

  for (const [collection, collectionEntries] of Array.from(entriesByCollection.entries())) {
    markdown += `## ${collection} (${collectionEntries.length})\n\n`;
    for (const entry of collectionEntries) {
      markdown += `### ${entry.author ? `${entry.author}` : "Unknown Author"}`;
      if (entry.work) markdown += ` — ${entry.work}`;
      markdown += `\n\n`;
      markdown += `> ${entry.text}\n\n`;
      if (template === "research" && entry.note) markdown += `**Note:** ${entry.note}\n`;
      if (entry.location) markdown += `**Location:** ${entry.location}\n`;
      if (entry.tags) markdown += `**Tags:** ${entry.tags.split(",").map((t: string) => `\`${t.trim()}\``).join(" ")}\n`;
      markdown += `\n`;
    }
  }
  ...
  return {
    format: "markdown",
    filename: `notebook_export_${new Date().toISOString().split("T")[0]}.md`,
    content: markdown,
    metadata: {
      exportDate: new Date().toISOString(),
      count: entries.length,
...
function buildPdfBuffer(render: (doc: InstanceType<typeof PDFDocument>) => void): Promise<string> {
...
export async function exportBooksAsPDF(userId: number, filters?: any) {
  const books = await listBooks(userId, filters);
  const template = filters?.template ?? "annotated";
  const base64 = await buildPdfBuffer((doc) => {
    doc.fontSize(22).text(template === "catalog" ? "Book Catalog" : "Book Collection", { align: "left" });
    doc.moveDown(0.5);
    doc.fontSize(10).fillColor("#666666").text(`Export Date: ${new Date().toLocaleDateString()}`);
    doc.text(`Total Books: ${books.length}`);
    doc.fillColor("#000000");
    doc.moveDown();

    books.forEach((book, index) => {
      if (index > 0) doc.moveDown();
      doc.fontSize(14).text(book.title || "Untitled", { underline: true });
      doc.fontSize(11);
      if (book.author) doc.text(`Author: ${book.author}`);
      if (book.status) doc.text(`Status: ${book.status}`);
      if (book.rating) doc.text(`Rating: ${book.rating}`);
      if (book.readingProgress !== undefined && book.readingProgress !== null) doc.text(`Reading Progress: ${book.readingProgress}%`);
      if (book.tags) doc.text(`Tags: ${book.tags}`);
      if (template === "annotated" && book.notes) {
        doc.moveDown(0.25);
        doc.text(`Notes: ${book.notes}`);
      }
      if (doc.y > 720 && index < books.length - 1) {
        doc.addPage();
      }
    });
  });
  ...
  return {
    format: "pdf",
    filename: `books_export_${new Date().toISOString().split("T")[0]}.pdf`,
    base64,
    metadata: {
      exportDate: new Date().toISOString(),
      count: books.length,
...
export async function exportNotebookEntriesAsPDF(userId: number, filters?: any) {
  const entries = await getNotebookEntries(userId, filters);
  const template = filters?.template ?? "reader";
  const base64 = await buildPdfBuffer((doc) => {
    doc.fontSize(22).text(template === "research" ? "Research Notebook" : "Notebook Entries", { align: "left" });
    doc.moveDown(0.5);
    doc.fontSize(10).fillColor("#666666").text(`Export Date: ${new Date().toLocaleDateString()}`);
    ...
    entries.forEach((entry, index) => {
      if (index > 0) doc.moveDown();
      doc.fontSize(12).text(entry.author || "Unknown Author", { underline: true });
      if (entry.work) doc.fontSize(11).text(`Work: ${entry.work}`);
      if (entry.location) doc.fontSize(11).text(`Location: ${entry.location}`);
      if (entry.collections) doc.fontSize(11).text(`Collections: ${entry.collections}`);
      doc.moveDown(0.25);
      doc.fontSize(11).text(entry.text || "", { align: "left" });
      if (template === "research" && entry.note) {
        doc.moveDown(0.25);
        doc.fontSize(10).fillColor("#444444").text(`Note: ${entry.note}`);
        doc.fillColor("#000000");
      }
      if (doc.y > 720 && index < entries.length - 1) {
        doc.addPage();
      }
    });
  });
  ...
  return {
    format: "pdf",
    filename: `notebook_export_${new Date().toISOString().split("T")[0]}.pdf`,
    base64,
    metadata: {
      exportDate: new Date().toISOString(),
      count: entries.length,
